...
public class gdxgame extends ApplicationAdapter {
	...
	public void render () {
		...
		//This is necessary because the coordinate system in which touch/mouse coordinates are reported might be different than the coordinate system we use to represent objects in our world.
		...
		/*
		 * We want the bucket to move without acceleration, 
		 * at two hundred pixels/units per second, either to the left or the right.
		 *  To implement such time-based movement we need to know the time that passed in between the last and
		 *   the current rendering frame. Here's how we can do all this:
		 */
		 if(Gdx.input.isKeyPressed(Keys.LEFT)) bucket.x -= BUCKET_SPEED * Gdx.graphics.getDeltaTime();
		 if(Gdx.input.isKeyPressed(Keys.RIGHT)) bucket.x += BUCKET_SPEED * Gdx.graphics.getDeltaTime();
